...
public class SystemLogger {
     ...
     * 
     * @param trace 调用者信息
     * @param excuteNo 执行编号 excuteNo String 系统执行编号，每个系统自定义 Y
     * @param executeName 执行名称 executeName String 某某功能或者某某线程等。 Y
     * @param executeDate 执行时间 reqTime String yyyyMMddhhmmss（ms） N 执行描述 executeDesc String 执行情况描述 N
     * @param executeStatus 执行状态 status String N
    ...
    public static void info(String executeDesc) {
        SystemLogger.info("", "", DateUtil.getCurrentTime(), executeDesc, "", 0);
    }
